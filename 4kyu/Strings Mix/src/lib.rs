...
fn mix(s1: &str, s2: &str) -> String {    
    let mut weighted_chars = ALPHABETH.chars().filter_map( |c| {
        ...
        if c1 > c2 { Some(format!("1:{}", &std::iter::repeat(c.to_string()).take(c1 as usize).collect::<String>())) }
        else if c2 > c1 { Some(format!("2:{}", &std::iter::repeat(c.to_string()).take(c2 as usize).collect::<String>())) }
        else { Some(format!("=:{}", &std::iter::repeat(c.to_string()).take(c1 as usize).collect::<String>())) }
    }).collect::<Vec<String>>();
